...
/***/ (function(module, exports, __webpack_require__) {
  ...
  AFRAME.registerComponent('teleport-controls', {
    ...
    init: function () {
      ...
      // hacked in for cancelling button events when intersecting interactive element
      


      // this doesn't work because they are gltf files and dont work like everything else.
      // hopefully we'll be good enough with the container boxes
      // collisionEntities = [].slice.call(el.sceneEl.querySelectorAll('.animated-model'));
      // this.avoidEntities .push(collisionEntities);
    },
    ...
    tick: (function () {
      ...
      return function (time, delta) {
        if (!this.active || this.data.disabled) { return; }
    ...
    onButtonDown: function () {
      ...
      if (this.checkNoShowCollisions()){
        // console.log(this.checkNoShowCollisions());
...
//
    ...
    checkNoShowCollisions: function () {
      // hacked in to allow us to check for meshes we should not show the line on.
      // var p0 = new THREE.Vector3();
// var next = new THREE.Vector3();
// var last = new THREE.Vector3();
      var quaternion = new THREE.Quaternion();
      ...
      var scale = new THREE.Vector3();
        ...
        this.obj.getWorldPosition(worldPosition);
      ...
      this.avoidEntities = [].slice.call(this.el.sceneEl.querySelectorAll('.no-teleport'));
      var meshes = this.avoidEntities.map(function (entity) {
